...
//
// Slightly modified for iStumbler appcast feeds by chainging the appname
// version seperator from '_' to '-' alf@istumbler.net 1/14/06
...
- (NSString *)newestRemoteVersionStringInFeed:(RSS *)feed
{
	...
	//    that it will look like this: http://something.com/YourApp-0.5.zip It'll read whatever's between the last
	//    underscore and the last period as the version number. So name your packages like this: APPNAME-VERSION.extension.
	...
	if (!newVersion) // no version attribute
	{
		...
		NSString *versionAndExtension = [[[enclosure objectForKey:@"url"] componentsSeparatedByString:@"-"] lastObject];
...
// #warning TODO: Handle caching / HTTP headers to see if the request is really necessary.
	...
	NS_HANDLER
		shouldContinue = NO;
		if ([[localException name] isEqualToString:@"RSSDownloadFailed"]
         || [[localException name] isEqualToString:@"RSSNoData"])
		{
